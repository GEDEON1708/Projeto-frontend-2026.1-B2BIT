...
export class PostService {
  static getAll(page: number = 1, limit: number = 10, search?: string) {
    const queryPage = Number.isNaN(page) ? 1 : page;
    ...
    if (search) {
      queryStr += ` WHERE p.title LIKE ? OR p.content LIKE ? OR u.name LIKE ? `;
    ...
    if (search) {
      countQuery += " JOIN users u ON p.authorId = u.id WHERE p.title LIKE ? OR p.content LIKE ? OR u.name LIKE ?";
      const total = (db.prepare(countQuery).get(`%${search}%`, `%${search}%`, `%${search}%`) as any)
        .total;
  ...
  static toggleLike(postId: number, userId: number) {
    const existingLike = db.prepare("SELECT id FROM likes WHERE postId = ? AND userId = ?").get(
      postId,
      userId,
    );
